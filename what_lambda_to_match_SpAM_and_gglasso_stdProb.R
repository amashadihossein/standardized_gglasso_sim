...
# QUESTION: Can we find the scale of hte lambda where 
#           standardized expanded X will give the same soln 
#           via gglasso or SpAM:
# Answer seems to be:
# for SpAM feed this lambda: sqrt(3)* lmbs
# for gglasso feed this lambda: lmbs/sqrt(nrow(X0)
#---------------------------------------------------------
...
# Comparison plot
#----------------
plot(c(cf),c(cf.gg),main="gglasso coefs vs SpAM \n Standardized problem\n (coef for intercept seems off when intercept=F for gglasso)",xlab = "Spam",ylab="gglasso")
abline(coef(lm(c(cf)~c(cf.gg))))
coefs <- as.character(signif(coef(lm(c(cf)~c(cf.gg))),3))
